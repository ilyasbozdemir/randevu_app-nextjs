...
export default function Dashboard() {
  ...
  const loadUserData = (userId: string) => {
    ...
    if (employeesJson) {
      ...
      localStorage.setItem(`employees_${userId}`, JSON.stringify([defaultEmployee]))
    }
    ...
    if (servicesJson) {
      ...
      localStorage.setItem(`services_${userId}`, JSON.stringify(defaultServices))
    }
  }
  ...
  useEffect(() => {
    if (currentUser) {
      localStorage.setItem(`employees_${currentUser.id}`, JSON.stringify(employees))
    }
  ...
  const saveAppointment = (appointment: Appointment) => {
    ...
    if (appointment.id) {
      ...
      setAppointments(appointments.map((app) => (app.id === appointment.id ? appointmentWithUserId : app)))
    } else {
  ...
  const saveEmployee = (employee: Employee) => {
    ...
    if (employee.id) {
      ...
      setEmployees(employees.map((e) => (e.id === employee.id ? employeeWithUserId : e)))
    } else {
  ...
  const deleteEmployee = (id: string) => {
    ...
    setAppointments(appointments.map((app) => (app.employeeId === id ? { ...app, employeeId: "" } : app)))
  }
  ...
  const saveService = (service: Service) => {
    ...
    if (service.id) {
      ...
      setServices(services.map((s) => (s.id === service.id ? serviceWithUserId : s)))
    } else {
  ...
  const deleteService = (id: string) => {
    ...
    setAppointments(appointments.map((app) => (app.serviceId === id ? { ...app, serviceId: "" } : app)))
  }
    ...
    <div className="container mx-auto py-4 px-2 md:py-6 md:px-4">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Hoş geldiniz, {currentUser.name}</p>
        </div>
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
          ...
          <Button variant="ghost" size="sm" onClick={handleLogout} className="flex-1 md:flex-none">
      ...
      <Tabs defaultValue="dashboard" className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                ...
                {upcomingAppointments.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {upcomingAppointments.map((appointment) => {
                      ...
                      return (
                        <div key={appointment.id} className="flex justify-between items-start border-b pb-3">
                          <div>
                            <p className="font-medium">{appointment.title}</p>
                            <p className="text-sm text-muted-foreground">
                              {format(appointment.date, "dd MMM yyyy", { locale: tr })} - {appointment.time}
                            </p>
                            {customer && <p className="text-sm">Müşteri: {customer.name}</p>}
                            {employee && <p className="text-sm">Çalışan: {employee.name}</p>}
                            {service && <p className="text-sm">Hizmet: {service.name}</p>}
                          </div>
                          <Button variant="ghost" size="sm" onClick={() => editAppointment(appointment)}>
                            Düzenle
                          </Button>
                        </div>
                      )
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <Button className="w-full" variant="outline" onClick={addNewCustomer}>
                  Yeni Müşteri Ekle
                </Button>
                <Button className="w-full" variant="outline" onClick={addNewEmployee}>
                  Yeni Çalışan Ekle
                </Button>
              </CardContent>
            </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Randevu eklemek için bir tarih seçin</CardDescription>
              </CardHeader>
              <CardContent className="flex justify-center">
                <Calendar mode="single" selected={date} onSelect={setDate} locale={tr} className="rounded-md border" />
              </CardContent>
            </Card>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>Müşteri listesi ve iletişim bilgileri</CardDescription>
              </div>
        ...
        <TabsContent value="employees">
          <Card>
            ...
            <CardContent>
              <EmployeeList employees={employees} services={services} onEdit={editEmployee} onDelete={deleteEmployee} />
            </CardContent>
          </Card>
        </TabsContent>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>Sunulan hizmetler ve fiyatları</CardDescription>
              </div>
        ...
        <TabsContent value="settings">
          <Card>
            <CardHeader>
              ...
              <CardDescription>İşletme bilgilerinizi ve ayarlarınızı düzenleyin</CardDescription>
            </CardHeader>
            <CardContent>
